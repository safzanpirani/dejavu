...
Agents lose useful context when work moves between Claude Code, Codex, Pi, and OpenCode. `deja` gives them one local command for finding earlier sessions and project memories.

`deja` is agent-first. An agent can search past sessions, inspect the relevant conversation, and recover decisions, commands, errors, and file changes. The agent can then verify that historical context against the current workspace. Humans can run the same commands from a terminal.

Search and transcript parsing stay on your machine. The optional `deja query` command sends selected conversation context to your configured Pi model. Search results can contain credentials or personal data that appeared in a transcript. Agents should treat the output as private.
...
```bash
deja 'Cannot find module'
deja find deployment timeout --since 2w
deja show '<locator from search results>' --around timeout
deja memory search 'deployment boundary'
```

Ask a model to summarize one selected session:

```bash
deja query '<locator from search results>' 'What did we decide?'
```

`deja query` requires an installed [Pi coding agent](https://github.com/earendil-works/pi). Plain search, `find`, `show`, and memory commands do not invoke a model.

## What it reads

`deja` detects these local stores:
...
```bash
deja --source codex session-recall.ts --max-parallel 4 --json
```
...
deja find deployment timeout --project payments-api --since 2w
```
...
```bash
deja show '<locator>'
deja show '<locator>' --around database
```
...
deja memory search 'deployment boundary' --json
deja memory show '<project or file selector>'
```
...
`deja query` follows the source's conversation structure. It removes reasoning, developer instructions, and tool output before it invokes Pi. Large sessions use windows around the question terms.
...
Run `deja --help` for the complete flag reference.
